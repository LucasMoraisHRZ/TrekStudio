...
const ServiceCard = ({
  ...
  return <div ref={cardRef} className="bg-white p-8 rounded-lg shadow-lg opacity-0 translate-y-10 transition-all duration-700 ease-out flex flex-col h-full">
      <div className="bg-[#00D781]/10 rounded-full p-4 mb-4 inline-block">
        <div className="text-[#00D781]">
          {icon}
        </div>
      </div>
      <h3 className="text-[#013334] mb-3 font-bold text-5xl">{title}</h3>
...
const Services = () => {
  ...
  return <section id="services" ref={sectionRef} className="section-spacing bg-[#013334] opacity-0 transition-opacity duration-1000 ease-out">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => <ServiceCard key={index} icon={service.icon} title={service.title} description={service.description} delay={service.delay} />)}
        </div>
      </div>
    </section>;
};
